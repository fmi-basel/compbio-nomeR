#' Bayesian inference of footprint abundance using No-U-Turn HMC sampler implemented in STAN
#'
#' This function utilizes rstan functionality for Bayesian inference, specifically employing Hamiltonian Monte Carlo (HMC) and No-U-Turn Sampler (NUTS) algorithms to infer footprint abundance in single-molecule footprinting data, such as NOMe-seq, by sampling from the posterior distribution.
#'
...
#' @param rstan_control A list of tuning parameters for the stan sampler algorithm.
#'
#' @param ... Parameters for \code{\link[rstan]{sampling}} function. Please refer to \code{\link[rstan]{sampling}} documentation.
#'
#' @return An S4 class stanfit-class representing the inference results. Please check \code{\link[rstan]{sampling}}.
#'
#' @export
#'
#' @examples
...
#' hmc_output <- infer_footprints_sampling(cooc_ctable = ftp_5_10_data,
#'                                        ftp_lengths = 2:15)
#'                                       
...
infer_footprints_sampling <- function(cooc_ctable,
                                      ftp_lengths,
                                      bg_prior_cover = 0.5,
  ...
  stan_initvals <- .infer_initialize_from_prior(dir_alpha = stan_input[["stan_inputdata"]][["ftp_prior_alphas"]],
                                                nchains = nchains,
                                                ftp_background_model = ftp_bg_model,
                                                background_model_params = bg_model_params,
