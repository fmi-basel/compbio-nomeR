...
#'   containing read-level data, such as that returned by
...
#' @param returnAs Character scalar. Return output as a \code{SummarizedExperiment}
#'   (\code{"SE"}) or as a list of \code{data.table}s (\code{"data.table"}).
#' @param profile Logical. If \code{TRUE}, enables time profiling of internal steps.
...
#'     reports the original modification probabilities from the input \code{se} object.
#'     }
    ...
    if (all(c(!is.null(predict_res_list[["START_PROB"]]),
        ...
        seOut <- .time_block({
            ...
            seOutRowRanges <- GenomicRanges::GPos(seqnames = posuniq[["chr"]],
                                                  ...
                                                  seqinfo = GenomeInfoDb::seqinfo(se))
