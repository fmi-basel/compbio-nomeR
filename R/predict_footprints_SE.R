...
#'     missing in the \code{footprint_models} probabilities are reported for 
#'     each individual footprints NAME defined in the \code{footprint_models}.
...
predict_footprints_SE <- function(se,
    ...
    prob_group <- fragpos <- posidx_ref <- fidx_glob <- sidx <- fidx_sample <- 
        chr <- refpos <- pos <- mod_prob <- gpos_idx <- ftp_name <- 
        ftp_group <- readName <- sname <- NULL # due to NSE notes in R CMD check
    ...
    if (all(c(!is.null(predict_res_list[["START_PROB"]]),
        ...
        predict_res <- 
            predict_res[, refpos := pos - 1 + 
                            fragAnno[match(seq,fragAnno[["fidx_glob"]])][["refStart"]] ]
        
        ## add chr, strand, sidx, and fidx_sample
        predict_res <- fragAnno[, list(fidx_glob, sidx, fidx_sample,
                                       chr, strand)][predict_res,
                                                     on = list(fidx_glob = seq)]
        
        ## add modprob
        predict_res <- 
            protect_data[, list(fidx_glob, fragpos, 
                                mod_prob)][predict_res, 
                                           on = list(fidx_glob = fidx_glob,
                                                     fragpos = pos)]
        fcols <- c("prob_group", "fidx_glob", "sidx", "fidx_sample", "fragpos",
                   "chr", "refpos", "strand", "mod_prob")
        setcolorder(predict_res, 
                    c(fcols, setdiff(colnames(predict_res), fcols)))
        
        ## create rowRanges
        posuniq <- unique(
            predict_res[, list(chr, refpos,strand)])[, gpos_idx := 1:.N]
        ## add gposidx
        predict_res <- posuniq[predict_res,
                               on = list(chr = chr, refpos = refpos, 
                                         strand = strand)]
        
        seOutRowRanges <- GenomicRanges::GPos(
            seqnames = posuniq[["chr"]],
            pos = posuniq[["refpos"]],
        ...
        nomeR_assayNames <- c("mod_prob",
                              paste(rep(ftpnames, 2),
        ...
        assayList <- lapply(
            ...
            function(assayI) {
                ...
                for (sI in seq_len(ncol(se))) {
                    ...
                    curDat <- predict_res[sidx == sI & 
                                              prob_group == curAssayName]
                    curDat <- curDat[!is.na(get(curFtpName))]
                    ### select which fragments belong to current sample and 
                    ### keep only those which passed the filtering
                    ...
                    namat <- NaArray(
                        dim = c(length(seOutRowRanges), nrow(curSmpFrags)),
        ...
        ## add reference positions
        viterbi_conf <- 
            viterbi_conf[, refpos := start - 1 + 
                             fragAnno[match(
                                 seq, fragAnno[["fidx_glob"]])][["refStart"]]]
        
        ## add sidx, fidx_sample, readName
        viterbi_conf <- 
            fragAnno[, list(fidx_glob, sidx, 
                            fidx_sample, readName)][viterbi_conf,
                                                    on = list(fidx_glob = seq)]
        coldat <- colData(se)
        ## add sample names
        ...
        mtdat$readLevelData$colDataColumns <- 
            c(mtdat$readLevelData$colDataColumns,
