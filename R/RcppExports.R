# Generated by using Rcpp::compileAttributes() -> do not edit by hand
# Generator token: 10BE3573-1514-4C36-9D1C-5A225CD40393

calcStartCoverProbs_cpp <- function(fragIDs, fragPos, protectVec, binding_models, bgprotectprob, bgprior, ftpConfigMethod, aggrByGroup, Ncpu, verbose) {
    .Call(`_nomeR_calcStartCoverProbs_cpp`, fragIDs, fragPos, protectVec, binding_models, bgprotectprob, bgprior, ftpConfigMethod, aggrByGroup, Ncpu, verbose)
}

count_spacing_freq_cpp <- function(fragIDs, fragPos, protectVec, maxspacing, Ncpu, verbose) {
    .Call(`_nomeR_count_spacing_freq_cpp`, fragIDs, fragPos, protectVec, maxspacing, Ncpu, verbose)
}

calculate_theor_joint_prob_cpp <- function(ftp_cover_priors, bg_protect_prob, footprint_protect_prob, max_spacing) {
    .Call(`_nomeR_calculate_theor_joint_prob_cpp`, ftp_cover_priors, bg_protect_prob, footprint_protect_prob, max_spacing)
}

