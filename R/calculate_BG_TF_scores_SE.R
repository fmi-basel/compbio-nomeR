...
#' @param bgAssayNames vector of assay names that contain predicted coverage probabilities for background (accessible state)
#' @param tfAssayNames vector of assay names that contain predicted coverage probabilities for transcription factors
#' @param nuclAssayNames vector of assay names that contain predicted coverage probabilities for nucleosomes
#' @param psc pseudo-count to avoid log(0)
#'
#' @returns \code{SummarizeExperiment} object with BG and TF scores stored as assays
#'
#' @importFrom SummarizedExperiment SummarizedExperiment assay assayNames colnames
...
calculate_BG_TF_scores_SE <- function(se,
    ...
    ## check assay names
    assert_vector(x = bgAssayNames,any.missing=FALSE,min.len = 1,unique=TRUE,null.ok=FALSE)
    assert_vector(x = tfAssayNames,any.missing=FALSE,min.len = 1,unique=TRUE,null.ok=FALSE)
    assert_vector(x = nuclAssayNames,any.missing=FALSE,min.len = 1,unique=TRUE,null.ok=FALSE)
    assert_number(x=psc,lower = 0,finite = TRUE)

    stopifnot(all(c(bgAssayNames,tfAssayNames,nuclAssayNames) %in% assayNames(se)))
