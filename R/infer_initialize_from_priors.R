...
.init_param_from_prior_distr <- function(stan_input,
  ...
  dirich_alpha <- stan_inputdata[["ftp_prior_cover"]] * stan_inputdata[["total_cnt_prior_dirich"]]
  ...
  init_vals <- lapply(1:nchains,function(x){
    ...
    if(stan_model_name == "ftp_inference_informative_prior"){
      bg_protect_alpha <- stan_inputdata[["bg_protect_mean"]] * stan_inputdata[["bg_protect_totcount"]]
      bg_protect_beta <- stan_inputdata[["bg_protect_totcount"]] * (1 - stan_inputdata[["bg_protect_mean"]])
      ftp_protect_alpha <- stan_inputdata[["ftp_protect_mean"]] * stan_inputdata[["ftp_protect_totcount"]]
      ftp_protect_beta <- stan_inputdata[["ftp_protect_totcount"]] * (1 - stan_inputdata[["ftp_protect_mean"]])
