#' Extract and/or plot estimates from footprint inference and (optionally) identify potential footprints from spectrum
#'
#' This utility function extract estimates from \code{infer_stanfit} and uses function \code{\link{suggest_footprints}} (optionally)
#' to detect peaks in spectrum and return lengths of potential footprints.
#'
#' @param infer_stanfit \code{\link[rstan]{stanfit}} object returned by \code{\link{infer_footprints_vb}},
#' \code{\link{infer_footprints_sampling}} or a \code{list} returned by \code{\link{infer_footprints_optim}}
#' @param ftp_abundance_name \code{character} which defines which footprint abundance value to return/display. Currently only ftp_abundances is accepted.
...
get_ftp_inference_summary <- function(infer_stanfit,
  ...
  if(!is.null(attr(infer_stanfit,"ftp_lengths"))){
  	...
  	stop("ERROR: infer_stanfit must have attribute ftp_lengths ")
  }
  ...
  if(inherits(infer_stanfit,"stanfit")){
    ...
    infer_ftp_abund_probs$ftp_length <- ftp_lengths[as.numeric(gsub("[\\[\\]]","",stringr::str_extract(row.names(infer_ftp_abund_probs),pattern = "\\[(\\d+)\\]"),perl=T))]
    ...
    if(infer_stanfit@model_name == "ftp_inference_informative_prior"){
      ...
      warning(paste0("Model name: ",infer_stanfit@model_name,". Setting infer_bg_protect_prob, infer_ftp_protect_prob to NA"))
      infer_bg_protect_prob <- NA
