...
#'
#' @param region_len Integer. Length of the simulated region (number of positions).
#' @param n_reads Integer. Number of molecules to generate.
#' @param footprint_models A \code{list} describing the footprints to generate.
#'     Each element is itself a \code{list} that must contain:
#'     \describe{
#'     \item{\code{PROTECT_PROB}}{Numeric vector of emission probabilities for
#'     protected positions, with length equal to the footprint length.}
#'     \item{\code{COVER_PROB}}{Numeric scalar giving the prior coverage
#'     probability for the footprint.}
#'     \item{\code{NAME}}{Character scalar: name of the footprint.}
#'     }
#' @param ftp_emission_posprob A numeric \code{matrix} of positional start
#'     probabilities for each footprint. The number of rows must equal the
#'     number of footprints and the number of columns must equal \code{region_len}.
#' @param bgprotectprob Numeric. Emission probability for background (accessible)
#'     positions.
#' @param infposdens If a scalar in \code{(0, 1]}, treated as the proportion of
#'     positions that are informative (observable). If a vector of integers,
#'     treated as the explicit indices of informative positions.
#'
...
generate_insilico_SMF_data <- function(region_len, # length of the amplicon
    ...
    if (length(infposdens) == 1 && all(infposdens > 0) && 
        ...
        if (any(!infposdens %in% seq_len(region_len))) {
            warning("infposdens contains positions outside amplicon ",
    ...
    if (bgcoverprior < 0) {
        stop("Sum of COVER_PRIOR across all footprint models exceeds 1. ",
             "Background cover prior would be negative (", bgcoverprior, ").")
    }
