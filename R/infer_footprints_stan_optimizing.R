...
#'
#'
#' @param ... parameters for `rstan::optimizing` function. Please check `?rstan::optimizing`.
#'
#' @return List returned by `rstan::optimizing`. Pleae check \code{?`rstan::optimizing`}. 
#' @export
#'
#' @examples
#' 
...
infer_footprints_stan_optimizing <- function(joint_freq_table,
                                     ...
                                     background_model = c("fixed","informative_prior"),
  ...
  ## Stan optimizing
  stan_optim_out <- rstan::optimizing(object = stanmodels[[stan_model_name]],
                                  data = stan_inputdata,
