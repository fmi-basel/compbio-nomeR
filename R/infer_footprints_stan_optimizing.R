...
#'
#'
#' @param ... parameters for `rstan::optimizing` function. Please check \code{\link[rstan]{optimizing}}.
#'
#' @return List returned by `rstan::optimizing`. Please check \code{\link[rstan]{optimizing}}.
#' @export
#'
#' @examples
#' 
...
#' 
#' 
#' ## point estimation for footprints in the data
#' optim_output <- infer_footprints_stan_optimizing(joint_freq_table = ftp_5_10_data,
...
infer_footprints_stan_optimizing <- function(joint_freq_table,
  ...
  stan_input <- validate_construct_stan_input(joint_freq_table,
                                              ...
                                              ftp_model_params)
  ## get initial values for optimizing
  ...
  ## Stan optimizing
  stan_optim_out <- rstan::optimizing(object = stanmodels[[stan_input$stan_model_name]],
                                  data = stan_input$stan_inputdata,
                                  init = stan_initvals[[1]],
                                  ...)
  
  return(stan_optim_out)
}