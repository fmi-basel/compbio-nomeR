## validate and construct input for inference
#' @importFrom checkmate makeAssertCollection test_data_frame assert_data_frame
#'     assert_names test_list assert_list test_names assert_integerish
#'     assert_number assert_double assert check_number reportAssertions
#'     testMatrix
.validate_construct_stan_input <- function(cooc_ctable,
                                           ftp_lengths,
                                           bg_prior_cover,
                                           ftp_prior_cover,
                                           total_cnt_prior_dirich,
                                           ftp_bg_model,
                                           bg_model_params,
                                           ftp_model_params) {
    #### CHECK ARGUMENTS ####
    coll <- makeAssertCollection()

    if (testMatrix(cooc_ctable, all.missing = FALSE, min.cols = 4)) {
        cooc_ctable <- data.frame(S = seq_len(nrow(cooc_ctable)),
                                  cooc_ctable)
    }

    ## validate count table
    if (test_data_frame(x = cooc_ctable,
                        min.rows = 1)) {
        ## check colnames
        assert_names(x = colnames(cooc_ctable),
                     must.include = c("S", "N00", "N01", "N10", "N11"),
                     add = coll)
        ## remove non-relevant columns
        cooc_ctable <- cooc_ctable[,c("S", "N00", "N01", "N10", "N11")]

        ## remove rows without data
        cooc_ctable <- cooc_ctable[rowSums(
            cooc_ctable[, c("N00", "N01", "N10", "N11")]) > 0, , drop = FALSE]

        ## check if anything left
        assert_data_frame(x = cooc_ctable,
                          types = "numeric",
                          min.rows = 1,
                          any.missing = FALSE,
                          add = coll)

        ## check vector S
        assert_integerish(x = cooc_ctable[, "S"],
                          lower = 1L,
                          any.missing = FALSE,
                          min.len = 1,
                          unique = TRUE,
                          add = coll)

        ## check if S=1 is in the table
        if (all(cooc_ctable[, "S"] != 1)) {
            stop("Incorrect cooc_ctable! Could not find a row with S = 1 ",
                 "which should represent total number of observed 0s and 1s ",
                 "in the data.")
        }

        ## sort table
        cooc_ctable <- cooc_ctable[order(cooc_ctable[, "S"]), ]
    } else {
        assert_data_frame(x = cooc_ctable, min.rows = 1, add = coll)
    }

    ## validate ftp_lengths
    assert_integerish(x = ftp_lengths,
                      lower = 2,
                      any.missing = FALSE,
                      min.len = 1,
                      unique = TRUE,
                      add = coll)

    ## validate bg_prior_cover
    assert_number(x = bg_prior_cover,
                  lower = 0, upper = 1,
                  add = coll)

    ## validate ftp_prior_cover
    if (is.null(ftp_prior_cover)) {
        ## if ftp_prior_cover is not provided use non-informative prior
        ftp_prior_cover <- rep((1 - bg_prior_cover) / length(ftp_lengths),
                               length(ftp_lengths))
    }
    assert_double(x = ftp_prior_cover, lower = 0, upper = 1,
                  finite = TRUE, any.missing = FALSE,
                  len = length(ftp_lengths), add = coll)
    ## validate total_cnt_prior_dirich
    if (is.null(total_cnt_prior_dirich)) {
        ## if total_cnt_prior_dirich is not provided choose such that footprint
        ## dirichlet alpha are all 1, i.e. non-informative dirichlet prior
        total_cnt_prior_dirich <- length(ftp_lengths) / (1 - bg_prior_cover)
    }
    assert_number(x = total_cnt_prior_dirich, lower = 1, add = coll)

    # validate bg_model_params
    if (test_list(bg_model_params,
                  types = c("double", "integerish"),
                  any.missing = TRUE, all.missing = FALSE)) {
        if (ftp_bg_model == "informative_prior") {
            if (test_names(
                x = names(bg_model_params),
                must.include = c("bg_protect_min", "bg_protect_max",
                                 "bg_protect_mean", "bg_protect_totcount"))) {
                assert(
                    check_number(
                        x = bg_model_params[["bg_protect_min"]],
                        lower = 0, upper = 1),
                    check_number(
                        x = bg_model_params[["bg_protect_max"]],
                        lower = 0, upper = 1),
                    check_number(
                        x = bg_model_params[["bg_protect_mean"]],
                        lower = 0, upper = 1),
                    check_number(
                        x = bg_model_params[["bg_protect_totcount"]],
                        lower = 0),
                    add = coll, combine = "and")
            } else {
                assert_names(
                    x = names(bg_model_params),
                    must.include = c("bg_protect_min", "bg_protect_max",
                                     "bg_protect_mean", "bg_protect_totcount"),
                    add = coll)
            }
        } else {
            if (test_names(
                x = names(bg_model_params),
                must.include = c("bg_protect_prob_fixed"))) {
                assert_number(
                    x = bg_model_params[["bg_protect_prob_fixed"]],
                    lower = 0, upper = 1, add = coll)
            } else {
                assert_names(
                    x = names(bg_model_params),
                    must.include = c("bg_protect_prob_fixed"),
                    add = coll)
            }
        }
    } else {
        assert_list(bg_model_params,
                    types = c("double", "integerish"),
                    any.missing = TRUE, all.missing = FALSE, add = coll)
    }

    ## validate ftp_model_params
    if (test_list(ftp_model_params,
                  types = c("double", "integerish"),
                  any.missing = TRUE, all.missing = FALSE)) {
        if (ftp_bg_model %in% c("informative_prior", "bg_fixed")) {
            if (test_names(
                x = names(ftp_model_params),
                must.include = c("ftp_protect_min", "ftp_protect_max",
                                 "ftp_protect_mean", "ftp_protect_totcount"))) {
                assert(
                    check_number(
                        x = ftp_model_params[["ftp_protect_min"]],
                        lower = 0, upper = 1),
                    check_number(
                        x = ftp_model_params[["ftp_protect_max"]],
                        lower = 0, upper = 1),
                    check_number(
                        x = ftp_model_params[["ftp_protect_mean"]],
                        lower = 0, upper = 1),
                    check_number(
                        x = ftp_model_params[["ftp_protect_totcount"]],
                        lower = 0),
                    add = coll, combine = "and")
            } else {
                assert_names(
                    x = names(ftp_model_params),
                    must.include = c("ftp_protect_min", "ftp_protect_max",
                                     "ftp_protect_mean", "ftp_protect_totcount"),
                    add = coll)
            }
        } else {
            if (test_names(
                x = names(ftp_model_params),
                must.include = c("ftp_protect_prob_fixed"))) {
                assert_number(
                    x = ftp_model_params[["ftp_protect_prob_fixed"]],
                    lower = 0, upper = 1, add = coll)
            } else {
                assert_names(
                    x = names(ftp_model_params),
                    must.include = c("ftp_protect_prob_fixed"),
                    add = coll)
            }
        }
    } else {
        assert_list(ftp_model_params,
                    types = c("double", "integerish"),
                    any.missing = TRUE, all.missing = FALSE, add = coll)
    }

    ## check if sum of bg prior and ftp is 1 and rescale if necessary
    prior_cover <- c(bg_prior_cover, ftp_prior_cover)
    totcover <- sum(prior_cover)
    if (!isTRUE(all.equal(totcover, 1))) {
        warning("Sum of prior coverages bg_prior_cover and ftp_prior_cover is ",
                totcover,
                " but should equal to 1. Rescale prior coverages accordingly.")
        prior_cover <- prior_cover / totcover
    }

    ## convert prior cover and tot counts to vector of dirichlet alphas
    ## update. in the v0.9 stan code the required data is prior_cover
    ## dirich_alpha <- prior_cover * total_cnt_prior_dirich
    ## add bg into vector of ftp lengths
    ftp_lengths <- c(1, ftp_lengths)

    ## calculate Beta distribution shape parameters
    ## update v0.9. the data does not require shape parameters.
    ## they are calculated in the stan code
    if (ftp_bg_model == "informative_prior") {
        stan_model_name <- "ftp_inference_informative_prior"
        stan_inputdata <- list(
            "n_ftp" = length(ftp_lengths),
            "ftp_lengths" = ftp_lengths,
            "ftp_prior_cover" = prior_cover,
            "total_cnt_prior_dirich" = total_cnt_prior_dirich,

            "n_spac" = nrow(cooc_ctable),
            "spacings" = cooc_ctable[, "S"],
            "spacing_counts" = cooc_ctable[, c("N00", "N01", "N10", "N11")],

            "bg_protect_min" = bg_model_params[["bg_protect_min"]],
            "bg_protect_max" = bg_model_params[["bg_protect_max"]],
            "bg_protect_mean" = bg_model_params[["bg_protect_mean"]],
            "bg_protect_totcount" = bg_model_params[["bg_protect_totcount"]],

            "ftp_protect_min" = ftp_model_params[["ftp_protect_min"]],
            "ftp_protect_max" = ftp_model_params[["ftp_protect_max"]],
            "ftp_protect_mean" = ftp_model_params[["ftp_protect_mean"]],
            "ftp_protect_totcount" = ftp_model_params[["ftp_protect_totcount"]])
    } else if (ftp_bg_model == "bg_fixed") {
        stan_model_name <- "ftp_inference_bg_fixed"

        stan_inputdata <- list(
            "n_ftp" = length(ftp_lengths),
            "ftp_lengths" = ftp_lengths,
            "ftp_prior_cover" = prior_cover,
            "total_cnt_prior_dirich" = total_cnt_prior_dirich,

            "n_spac" = nrow(cooc_ctable),
            "spacings" = cooc_ctable[, "S"],
            "spacing_counts" = cooc_ctable[, c("N00", "N01", "N10", "N11")],

            "bg_protect_prob" = bg_model_params[["bg_protect_prob_fixed"]],

            "ftp_protect_min" = ftp_model_params[["ftp_protect_min"]],
            "ftp_protect_max" = ftp_model_params[["ftp_protect_max"]],
            "ftp_protect_mean" = ftp_model_params[["ftp_protect_mean"]],
            "ftp_protect_totcount" = ftp_model_params[["ftp_protect_totcount"]])
    } else if (ftp_bg_model == "ftp_bg_fixed") {
        stan_model_name <- "ftp_inference_ftp_bg_fixed"
        stan_inputdata <- list(
            "n_ftp" = length(ftp_lengths),
            "ftp_lengths" = ftp_lengths,
            "ftp_prior_cover" = prior_cover,
            "total_cnt_prior_dirich" = total_cnt_prior_dirich,

            "n_spac" = nrow(cooc_ctable),
            "spacings" = cooc_ctable[, "S"],
            "spacing_counts" = cooc_ctable[, c("N00", "N01", "N10", "N11")],

            "bg_protect_prob" = bg_model_params[["bg_protect_prob_fixed"]],
            "ftp_protect_prob" = ftp_model_params[["ftp_protect_prob_fixed"]])
    } else {
        stop("Incorrect ftp_bg_model.")
    }

    reportAssertions(coll)
    return(list("stan_model_name" = stan_model_name,
                "stan_inputdata" = stan_inputdata))
}
