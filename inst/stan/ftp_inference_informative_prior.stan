...
This model is used for bayesian inference of footprint spectrum and bg and ftp emission probabilities.
For bg and ftp emission probabilities beta distributions are used as a prior.
...
data {
  ...
  vector[n_ftp] ftp_prior_cover; // vector of expected footprint coverages. this vector incudes 1, i.e. background and ith element corresponds to ith element in ftp_lengths
...
transformed data {
  ...
  // bg noise beta distr hyperparams
  real bg_protect_alpha = bg_protect_totcount * bg_protect_mean;
  real bg_protect_beta = bg_protect_totcount * (1 - bg_protect_mean);
